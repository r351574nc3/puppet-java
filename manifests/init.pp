...
#   - Module['Archive']
class java {
    $url32 = "http://download.oracle.com/otn-pub/java/jdk/7u25-b15/jdk-7u25-linux-i586.tar.gz"
    $url64 = "http://download.oracle.com/otn-pub/java/jdk/7u25-b15/jdk-7u25-linux-x64.tar.gz"
    ...
    archive::download { "jdk-7u25-linux-x64.tar.gz":
        ensure        => present,
        url           => $url32,
    ...
    archive::extract { "jdk-7u25-linux-x64" :
        ensure     => present,
        target     => "/usr/java",
        require    => Archive::Download["jdk-7u25-linux-x64.tar.gz"]
    }

    file { '/usr/java/latest':
        ensure => link,
        target => '/usr/java/jdk1.7.0_25',
    }
